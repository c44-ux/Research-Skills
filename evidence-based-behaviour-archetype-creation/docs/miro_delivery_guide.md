...
**Requirements:** Cursor with Miro MCP/plugin enabled, Miro login, and permission to create boards in your team (for path A).
...
- **Miro MCP** enabled in Cursor; user signed in to Miro (agent may ask them to complete login if auth fails).
- Agent follows the **miro-mcp** Cursor skill for tool calls.
