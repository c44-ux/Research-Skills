...
```text
Plan study → Collect data → Analyse in Claude → method-synopsis → Save playbook → Next wave reads prior playbook
```
...
.claude/
├── skills/
│   └── method-synopsis/           # copy from Research-Skills/method-synopsis/claude/
├── commands/
│   └── method-synopsis.md
...
|-------|----------------------------|---------------------|
| **evidence-archetypes** — quant | After survey Phase 3 scripts and segment reports | Column mapping path, script commands, segment rules, output paths |
| **evidence-archetypes** — qual | After thematic/archetype synthesis from transcripts | Participant scope, codebook, quote policy, Miro board URL |
...
1. Open or paste the **prior playbook** for the same `study_id`.
2. Run analysis in a new Claude chat.
3. Trigger **method-synopsis** — Claude populates **What changed vs last wave**.
...
- Playbooks in `.claude/method-playbooks/` may be committed for team reproducibility — use redaction and synthetic labels.
